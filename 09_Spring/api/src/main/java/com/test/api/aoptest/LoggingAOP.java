...
public class LoggingAOP {
	...
	/*
	 * <시점>
	 * @Beford : 대상메서드 실행 전에 Advice(추가기능)가 실행된다.
	 * 
	 * @After : 대상 메서드 실행 후에 Advice(추가기능)가 실행된다.
	 * 
	 * @AfterReturning : 대상 메서드가 정상적으로 반환된 후에 Advice(추가기능)가 실행된다.
	 * 
	 * @AfterThrowing : 대상 메서드가 예외를 던진 후 에 Advice(추가기능)가 실행된다.
	 * 
	 * @Around : 대상메서드를 감싸서 메서드 호출 전 후에 Advice를 실행할 수 있다.
	 */
	...
	//특정 메소드나 패키지등의 join point
	//com.kh아래 모든 controller패키지 하위 클래스의 모든 메서드에 전부 적용하겠다고 지점 설정.
	@Pointcut("execution(* com.test.api.aoptest.*.*(..) )")
	private void cut() {};
	...
	public Object displayLogInfo(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		log.info("Processiong Time : " + runTime + "ms");

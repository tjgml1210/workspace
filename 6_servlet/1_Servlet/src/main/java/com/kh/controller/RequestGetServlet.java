...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 첫번째 매개변수인 requst에는 요청시 전달된 내용들이 담겨있음(사용자가 입력한 값, 요청방식, 요청자의 ip주소 등)
		 * 두번째 매개변수인 response는 요청 처리 후 응답에 사용되는 객체
		 * 
		 * 요청처리를 위해서 요청 시 전달된 값들 추출
		 * request의 paramerter영역안에 존재
		 * request.getPrameter("키")
		 */
		
		String name = request.getParameter("name"); //"신서희" | ""
		...
		double height = Double.parseDouble(request.getParameter("height")); //"179" -> 179.99
		...
		//1) 이제부터 내가 출력할 내용은 문서형태의 html이고 문자셋은 urf-8이다 -> 선언
